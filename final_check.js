        ...
        function generateShortcutScript() {
            ...
            let scriptLines = [
                ...
                '    \$targetW = if (\$hasRotation) { \$sh } else { \$sw }',
                '    \$targetH = if (\$hasRotation) { \$sw } else { \$sh }',
                ...
                '        \$vFilters += "[0:v]trim=start_frame=\$startF:end_frame=\$endF,setpts=PTS-STARTPTS\$transpose\$flip\$cropFilter,scale=w=\$targetW:h=\$targetH:force_original_aspect_ratio=decrease,pad=\$targetW:\$targetH:(ow-iw)/2:(oh-ih)/2,setsar=1[\$vName]"',
                '        if (\$hasAudio) {',
                ...
                '            \$aFilters += "[0:a]atrim=start=\$vStart:end=\$vEnd,asetpts=PTS-STARTPTS[\$aName]"',
                '        }',
                '    }',
                ...
                '    \$fullFilter += \$finalA',
                '    \$fullFilter | Out-File -LiteralPath \$filterPath -Encoding utf8',
                '    ',
                '    Write-Host "  > Encoding with filter_complex..."',
                '    \$mapArgs = if (\$hasAudio) { @("-map", "[v]", "-map", "[a]") } else { @("-map", "[v]") }',
                '    ffmpeg -y -noautorotate -i "\$sourcePath" -filter_complex_script "\$filterPath" @mapArgs -c:v libx264 -crf 17 -preset veryslow -c:a aac -b:a 192k -metadata:s:v:0 rotate=0 -movflags +faststart "\$targetPath"',
